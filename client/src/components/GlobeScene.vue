<template>
  <div id="container" class="cursor-pointer" tabindex="-1" />
</template>
...
const minCameraDistance = sphereSize / 2 + 5;
...
const moveDampening = 0.5;
let moveXSpeed = 0;
let moveYSpeed = 0;
const moveSpeedMax = 0.01;
// Used by the SHIFT + LEFT/RIGHT keys and the ⟳/⟲ control buttons.
const rotationAcceleration = 0.03;
const rotationDampening = 0.2;
let rotationSpeed = 0;
const rotationSpeedMax = 2;
// Used by the -/+ keys and the -/+ control buttons.
const zoomAcceleration = 0.03;
const zoomDampening = 0.05;
let zoomSpeed = 0;
const zoomSpeedMax = 1;
const zoomScrollSpeed = 0.0005;
...
const mouseMoveDistanceThreshold = 2;
...
const touchMoveDistanceThreshold = 10;
...
const animateToSelectedSuggestedZoom: number = minCameraDistance + 10;
const animateToSelectedSuggestedMaxZoom: number = minCameraDistance + 150;
...
addUpdateBulkListener(() => {
  for (let i = 0; i < vertexColorIds.length; i += 3) {
    const colorId = stateColorIds[i / 3];
...
function getPointIntersectionAtScreenPosition(
  ...
  return raycaster.ray.intersectSphere(
    new THREE.Sphere(new THREE.Vector3(), sphereSize / 2),
    new THREE.Vector3(),
  );
}
...
function animateControls() {
  ...
  const moveSpeed1Fraction = Math.max(
    0.0001,
    (currentZoom - minCameraDistance) / (maxCameraDistance - minCameraDistance),
  );
  if (props.moveXDirection !== 0) {
    moveXSpeed += moveAcceleration * props.moveXDirection * moveSpeed1Fraction;
  } else {
  ...
  if (props.moveYDirection !== 0) {
    moveYSpeed += moveAcceleration * props.moveYDirection * moveSpeed1Fraction;
  } else {
  ...
  if (spinAngleMomentum > 0) {
    const spinAngleFraction = Math.max(
      0.01,
      (currentZoom - minCameraDistance) / (maxCameraDistance - minCameraDistance),
    );
    spinAngleMomentum -= spinMomentumDampening * spinAngleFraction;
  ...
  // Dampen the spin (Used when swiping the mouse or swiping the touch display)
  const zoomMomentumFraction = Math.max(
    0.01,
    (currentZoom - minCameraDistance) / (maxCameraDistance - minCameraDistance),
  );
  if (zoomMomentum > 0) {
    zoomMomentum -= zoomMomentumDampening * zoomMomentumFraction;
  ...
  // Clamp the speed. 🗜
  const moveSpeed2Fraction = Math.max(
    0.01,
    (currentZoom - minCameraDistance) / (maxCameraDistance - minCameraDistance),
  );
  moveXSpeed = clamp(
    moveXSpeed,
    -moveSpeedMax * moveSpeed2Fraction,
    moveSpeedMax * moveSpeed2Fraction,
  );
  moveYSpeed = clamp(
    moveYSpeed,
  ...
  if (zoomMomentum !== 0) {
    const newZoom = clamp(
      currentZoom * (1 + zoomMomentum * deltaTime),
      minCameraDistance,
      maxCameraDistance,
    );
  ...
  const animateToSelectedDuration = Math.min(
    1,
    (Date.now() - animateToSelectedTimeStart) / animateToSelectedTimeTotal,
  );

  if (animateToSelectedPosStart !== null && animateToSelectedPosEnd !== null) {
    const animateToSelectedTargetAxis = new THREE.Vector3().crossVectors(
      animateToSelectedPosStart,
      animateToSelectedPosEnd,
    ).normalize();
    const animateToSelectedTargetTotalAngle = (
      animateToSelectedPosStart.angleTo(animateToSelectedPosEnd)
    );
    const animateToSelectedTargetCurrentAngle = (
      animateToSelectedTargetTotalAngle - camera.position.angleTo(animateToSelectedPosEnd)
    );
    const interpolatedAngle = damp(
      animateToSelectedTargetCurrentAngle,
      animateToSelectedTargetTotalAngle,
      0.5,
      animateToSelectedDuration,
    );
    rotateCameraAlongAxis(
      camera,
      animateToSelectedTargetAxis,
      interpolatedAngle - animateToSelectedTargetCurrentAngle,
    );
  }

  if (animateToSelectedZoomStart !== null && animateToSelectedZoomEnd !== null) {
    const interpolatedZoom = lerp(
      animateToSelectedZoomStart,
      animateToSelectedZoomEnd,
      animateToSelectedDuration,
    );
  ...
  if (zoomSpeed !== 0) {
    const newZoom = clamp(
      currentZoom * (1 - zoomSpeed * deltaTime),
      minCameraDistance,
      maxCameraDistance,
    );
...
function mouseMoveEventHandler(event: MouseEvent) {
  ...
  if (mouseMoveDistance >= mouseMoveDistanceThreshold) {
    ...
    const axis = new THREE.Vector3().crossVectors(centerInt, centerDeltaInt).normalize();
    const angle = centerInt.angleTo(centerDeltaInt);
...
function touchMoveEventHandler(event: TouchEvent) {
  ...
  if (
    ...
    const centerPosition = getCenterScreenPosition();
    const centerDeltaPosition = new THREE.Vector2();
    const centerInt = getPointIntersectionAtScreenPosition(centerPosition);
  ...
  if (touch2 !== null && touchId2MovePos !== null) {
    ...
    if (
      ...
      if (sceneDeltaA !== null && sceneDeltaB !== null) {
        ...
        const zoomFraction = (1 - Math.min(
          0.9,
          (currentZoom - minCameraDistance) / (maxCameraDistance - minCameraDistance),
        ));
        const zoomFactor = (angle * zoomFraction) * 5;
...
function touchCancelEventHandler() {
...
watch(() => [
  props.moveXDirection,
  props.moveYDirection,
...
watch(() => [props.selectedPosition], () => {
  if (props.selectedPosition === null) {
    ...
    const v1 = new THREE.Vector3(
      // @ts-ignore While ArrayLike<number> does not have .slice(), Int32Array does.
      ...sphere.geometry.attributes.position.array.slice(faceIndex, faceIndex + 3),
    );
    const v2 = new THREE.Vector3(
      // @ts-ignore While ArrayLike<number> does not have .slice(), Int32Array does.
      ...sphere.geometry.attributes.position.array.slice(faceIndex + 3, faceIndex + 6),
    );
    const v3 = new THREE.Vector3(
      // @ts-ignore While ArrayLike<number> does not have .slice(), Int32Array does.
      ...sphere.geometry.attributes.position.array.slice(faceIndex + 6, faceIndex + 9),
    );
...
onMounted(() => {
  ...
  function createControls() {
    ...
    // touchEnd/mouseUp with no significant dragging should zoom + pan + focus (animated zoom).
    // - update selectedPosition
    // - Increase zoom level from the current zoom, with a maxZoom.
    ...
    // touchMove with two fingers should affect zoom.
